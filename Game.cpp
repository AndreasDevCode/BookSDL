...
bool Game::Initialize()
{
	...
	mWindow = SDL_CreateWindow("Game Programming in C++ Chap.3"
								,100,100,1024,768,0);
	if (!mWindow)
	{
		SDL_Log("Failed to create window: %s", SDL_GetError());
...
void Game::LoadData()
{
	...
	mShip-> SetRotation(Math::PiOver2);
	// load Asteroids
	const int numberOfAsteroids = 20;
...
SDL_Texture* Game::GetTexture(const std::string& fileName)
{
	...
	else
	{
		...
		if (!surf)
		{
			SDL_Log("Could not find imagefile: %s", fileName.c_str());
			return nullptr;
		}
		tex = SDL_CreateTextureFromSurface(mRenderer, surf);
		SDL_FreeSurface(surf);
...
void Game::AddAsteroids(Asteroid* as)
{
	mAsteroids.emplace_back(as);
}
void Game::RemoveAsteroids(Asteroid* as)
{
